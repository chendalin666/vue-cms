...
(function ($, window) {
  ...
  const calTouchData = function (touch) {
    ...
    const { firstTouch } = session;
    const { firstMultiTouch } = session;
  ...
  $.addInit({
    ...
    handle() {
      ...
      if (hasPulldown || hasPullup) {
        const { container } = pullRefreshOptions;
        if (container) {
...
(function ($, window, undefined) {
  ...
  var serialize = function (params, obj, traditional, scope) {
    let type; const array = $.isArray(obj);
  ...
  const Scroll = $.Class.extend({
    ...
    _initSnap() {
      ...
      for (let i = 0; i < length; i++) {
        const snap = snaps[i];
        const { offsetLeft } = snap;
        const { offsetWidth } = snap;
    ...
    _drag(e) {
      ...
      if (this.options.scrollY || detail.direction === 'up' || detail.direction === 'down') { // 如果是竖向滚动或手势方向是上或下
        ...
        if ($.os.ios && parseFloat($.os.version) >= 8) { // 多webview时，离开当前webview会导致后续touch事件不触发
          const { clientY } = detail.gesture.touches[0];
          // 下拉刷新 or 上拉加载
...
(function ($, window, document, undefined) {
  ...
  const PullRefresh = $.Scroll.extend($.extend({
    ...
    pulldownLoading(y, time) {
      ...
      const { callback } = this.options.down;
...
(function ($, document) {
  ...
  $.plusReady(() => {
    ...
    const PlusPullRefresh = $.Class.extend({
      ...
      _initPulldownRefreshEvent() {
        ...
        $.plusReady(() => {
          if (self.options.down.style == 'circle') {
            ...
            const webview = plus.webview.getWebviewById(self.options.webviewId); // 子窗口
            if (!webview) {
              return;
            }
            self.options.webview = webview;
            const downOptions = self.options.down;
            const { height } = downOptions;
      ...
      pulldownLoading() { // 该方法是子页面调用的，兼容老的历史API
        this.beginPulldown();
      },
      ...
      endPulldownToRefresh() { // 该方法是子页面调用的，兼容老的历史API
        this.endPulldown();
      },
      ...
      pullupLoading(callback) { // 兼容老的API
        this.beginPullup(callback);
      },
      endPullup(finished) { // 上拉加载结束
        const self = this;
        if (self.pullLoading) {
      ...
      endPullupToRefresh(finished) { // 上拉加载结束，兼容老的API
      ...
      enablePulldownToRefresh() {
        const self = this;
        const webview = plus.webview.currentWebview();
        const { height } = this.options.down;
        // 单webview模式禁止原生下拉刷新
